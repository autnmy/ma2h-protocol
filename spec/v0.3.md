...
|-------|------|-----|-------|
| `ahcp_version` | string | MUST | `"0.3"`. Version handling per §10. |
...
"request": {
  ...
  "schema": {                  // REQUIRED when mode=input — FLAT JSON Schema (string/number/bool/enum)
    "type": "object",
    "properties": { "reason": { "type": "string", "x-ahcp-sensitive": false } },
    "required": ["reason"]
  },
...
### 8.0 Discovery — `GET /.well-known/ahcp`
...
"callback": {
  ...
  "auth": { "scheme": "hmac", "secret_ref": "env:AHCP_CALLBACK_SECRET" }  // see §9.2
}
```
...
```
signed_context = {
  "ahcp_version", "callback_url", "id", "in_reply_to", "jti",
...
```
AHCP-Signature: t=<unix>,jti=<nonce>,v1=<base64url(signature)>
```
...
inside the `signed_context` covered by the `AHCP-Signature`
...
- **Telemetry.** Hub logs SHOULD exclude `body`, `context`, `response.value`, and `response.comment` by
  default, and MUST NOT log `state`. A property marked `x-ahcp-sensitive: true` (or a message-level
...
- `ahcp_version` is `MAJOR.MINOR`. The **major** is the integer before the first dot. A Hub MUST reject a
...
- **Push version parity (v0.3).** Because the break is in the *signature*, a v0.3 Hub's **pushed** Response
  ...
  a **push** callback at a pre-0.3 `ahcp_version` with `version_not_supported` — emitting a callback the
