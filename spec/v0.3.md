...
byte-for-byte. Because that number formatting is the ECMAScript `Number::toString` algorithm over IEEE-754
doubles, a signed numeric value MUST be exactly representable as a double: an integer outside ±(2^53−1)
MUST be carried as a **string**, not a number, or signer and verifier disagree. A non-JS implementation
MUST use a vetted JCS library (a JS `JSON.stringify` of a number already conforms); the `dp-004`
conformance vector pins the canonical bytes + digest for a numeric payload as the cross-language interop
target. `payload_sha256` is carried only inside the `signed_context` covered by the `A2H-Signature`
