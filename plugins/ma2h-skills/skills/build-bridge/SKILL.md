...
| `3` | own session terminal | drain/ack/register hit the own-terminal `410` with `error.code: gone` — the lease lapsed or the session was self-closed (§16.3). **Restart → register a fresh session → continue.** The only fatal class whose remedy is a restart. |
