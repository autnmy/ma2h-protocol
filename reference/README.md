...
`noUncheckedIndexedAccess`, `verbatimModuleSyntax`). All tests pass (290 at v0.5).
...
| `src/client.ts` | **v0.5 (#45): the vendored client layer, keyed side** — the `Agent` class (§13.4 duty sequence in its implemented order), the `BridgeHub` transport seam, entry taxonomy + ack keys, structured dispositions + the exhaustive `classifyEntryResult` verdict | ✅ |
| `src/wire.ts` | **v0.5 (#45): the vendored client layer, keyless side** — envelope builders + the feature→minimum-minor version rule, §8.1 ack/misroute validation (schema-delegated), per-type status tables (derivation-guarded), drain-batch shape guard, `validateKnownFields`, `effectiveCode` + the six-class `classifyHubError` §8.5 reading | ✅ |
...
```bash
cd reference && npm install && npm test        # tsx + node:test (290 tests)
