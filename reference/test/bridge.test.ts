...
// nonzero exit codes for auth failure / own-terminal session / signature failure, never a silent
// death.
...
test("exit code 3: the bridge's OWN session going terminal mid-run exits loudly and distinctly (§8.7.1's 410/§16.3)", () => {
  const now = { t: T0 };
  const hub = newHub(now);
  // The kill-switch (or a lease lapse) lands between the register and the drain: model it by
  // closing the session the moment it exists — the very first drain then reads the own-terminal 410.
...
test("the three exit codes are distinct and nonzero — a supervisor can tell the failures apart", () => {
  const codes = [EXIT_AUTH_FAILURE, EXIT_SESSION_TERMINAL, EXIT_SIGNATURE_FAILURE];
